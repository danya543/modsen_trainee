...
export const FavoritesPage = () => {
    ...
    useEffect(() => {
        fetchPosters(favorites).then((data) => setPosts([...data].sort((a, b) => isSortIncrease
            ? a.data.title.localeCompare(b.data.title) : b.data.title.localeCompare(a.data.title))));
    }, [favorites])

    useEffect(() => {
        if (posts) {
            const sortedData = [...posts].sort((a, b) => isSortIncrease
                ? a.data.title.localeCompare(b.data.title) : b.data.title.localeCompare(a.data.title));
            setPosts(sortedData);
        }
    }, [isSortIncrease])
